define(['./Transforms-e2d4a55a', './Matrix3-41c58dde', './ComponentDatatype-cf1fa08e', './defaultValue-fe22d8c0', './Check-6ede7e26', './GeometryAttribute-13da9466', './GeometryAttributes-ad136444', './IndexDatatype-2643aa47', './VertexFormat-030f11ff', './Math-0a2ac845', './Matrix2-e1298525', './RuntimeError-ef395448', './combine-d9581036', './WebGLConstants-0b1ce7ba'], (function (Transforms, Matrix3, ComponentDatatype, defaultValue, Check, GeometryAttribute, GeometryAttributes, IndexDatatype, VertexFormat, Math$1, Matrix2, RuntimeError, combine, WebGLConstants) { 'use strict';
  ...
  FanGeometry.createGeometry = function (fanGeometry) {
    ...
    for (i = 0; i < directionsLength; i++) {
      direction = Matrix3.Cartesian3.fromSpherical(sphericalDiretions[i]);
      if (i === 0) {
        directions.push(direction);
        normalizedDirections.push(
          Matrix3.Cartesian3.normalize(direction, new Matrix3.Cartesian3())
        );
      } else if (!Matrix3.Cartesian3.equals(directions[i - 1], direction)) {
        if (i === directionsLength - 1) {
          if (!Matrix3.Cartesian3.equals(directions[0], direction)) {
            directions.push(direction);
            normalizedDirections.push(
              Matrix3.Cartesian3.normalize(direction, new Matrix3.Cartesian3())
            );
    ...
    if (vertexFormat.position) {
      ...
      for (i = 0; i < directionsLength; i++) {
        ...
        const currentRadius = perDirectionRadius
          ? Matrix3.Cartesian3.magnitude(directions[i])
          : radius;
    ...
    if (vertexFormat.normal) {
      ...
      for (i = 0; i < directionsLength; i++) {
        ...
        scratchCartesian = Matrix3.Cartesian3.normalize(
          Matrix3.Cartesian3.cross(direction, direction2, scratchCartesian),
          scratchCartesian
        );
    ...
    if (vertexFormat.tangent) {
      ...
      for (i = 0; i < length; i += 6) {
        const normal = Matrix3.Cartesian3.unpack(normals, i);
        const bitangent = Matrix3.Cartesian3.unpack(bitangents, i);
        const tangent = Matrix3.Cartesian3.normalize(
          Matrix3.Cartesian3.cross(bitangent, normal, scratchCartesian),
          scratchCartesian
        );
