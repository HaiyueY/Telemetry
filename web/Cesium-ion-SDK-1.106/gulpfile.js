...
export async function build() {
  ...
  if (workspace === `@${scope}/engine`) {
    ...
    return buildWorkspace("widgets", buildOptions);
  } else if (workspace === `@${scope}/ion-sdk-widgets`) {
    return buildWorkspace("ion-sdk-widgets", buildOptions);
  }

  await buildEngine(buildOptions);
  await buildWorkspace("widgets", buildOptions);
  await buildWorkspace("ion-sdk-widgets", buildOptions);
...
async function clocSource() {
  ...
  const source = new Promise(function (resolve, reject) {
    cmdLine =
      ...
      " packages/engine/Source/ packages/widgets/Source packages/ion-sdk-widgets/Source --exclude-dir=Assets,ThirdParty,Workers";
  ...
  return new Promise(function (resolve, reject) {
    cmdLine =
      "npx cloc" +
      " --quiet --progress-rate=0" +
      " Specs/ packages/engine/Specs packages/widget/Specs packages/ion-sdk-widgets/Specs --exclude-dir=Data --not-match-f=SpecList.js --not-match-f=.eslintrc.json";
    exec(cmdLine, function (error, stdout, stderr) {
      if (error) {
        console.log(stderr);
        return reject(error);
...
export const buildRelease = gulp.series(
  buildEngine,
  () => buildWorkspace("widgets"),
  () => buildWorkspace("ion-sdk-widgets"),
...
const getWorkspaceFiles = (workspaceDirectory) => {
  const packageJson = require(`./${workspaceDirectory}/package.json`);
  return packageJson.files.map((pattern) => join(workspaceDirectory, pattern));
};
...
export const makeZip = gulp.series(release, async function () {
  ...
  for (const workspace of packageJson.workspaces) {
    ...
    const packageSource = await pruneScriptsForZip(
      join(workspace, "package.json")
    );
    packageJsonSources.push(packageSource);
  }
  ...
  return streamToPromise(
    mergeStream(
      ...
      .pipe(gulpZip(`Cesium-ion-SDK-${version}.zip`))
      .pipe(gulp.dest("."))
      .on("finish", async () => {
        const files = await globby([
...
export async function prepareDownload() {
  ...
  const zipName = `Cesium-ion-SDK-${version}.zip`;
  ...
  const stream = createReadStream(resolve("./IONCHANGES.md"));
  ...
  result.releaseNotes = converter.makeHtml(releaseNotes);
...
export async function coverage() {
  ...
  return runCoverage({
    ...
    filter: /packages(\\|\/)(engine|widgets|ion)(\\|\/)Source((\\|\/)\w+)+\.js$/,
    webglStub: argv.webglStub,
    suppressPassed: argv.suppressPassed,
    failTaskOnError: argv.failTaskOnError,
  });
...
function createTypeScriptDefinitions() {
  ...
  // Wrap the source to actually be inside of a declared cesium module
  // and add any workaround and private utility types.
  source = `declare module "@cesiumgs/cesium-analytics" {
${source}
}
