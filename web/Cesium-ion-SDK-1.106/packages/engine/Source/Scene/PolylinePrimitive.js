...
function PolylinePrimitive(options) {
  ...
  this._depthFailMaterialType = defaultValue(
    options.depthFailMaterialType,
    this._materialType
  );

  this._uniforms = defaultValue(options.uniforms, undefined);
  this._depthFailUniforms = defaultValue(
    options.depthFailUniforms,
    this._uniforms
  );
...
Object.defineProperties(PolylinePrimitive.prototype, {
  ...
  depthFailUniforms: {
    ...
    set: function (value) {
      ...
      const materialUniforms = this._primitive.depthFailAppearance.material
        .uniforms;
      for (const name in value) {
        if (value.hasOwnProperty(name)) {
          materialUniforms[name] = value[name];
