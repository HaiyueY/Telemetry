...
function ClippingPlanesEditor(options) {
  ...
  this._removeAddEventListener = clippingPlanes.planeAdded.addEventListener(
    function () {
      that._readyPromise.then(function () {
        that._addPlane();
      });
    },
    this
  ...
  this._readyPromise = new Promise(function (resolve, reject) {
    that._createPlanes(resolve, reject);
  });
}
...
Object.defineProperties(ClippingPlanesEditor.prototype, {
  ...
  /**
   * Gets the ready promise which resolves after the clipping plane owner is ready and planes have been added.
   * @type {Promise<ClippingPlanesEditor>}
   * @memberof ClippingPlanesEditor.prototype
   * @readonly
   * @deprecated
   */
  readyPromise: {
    get: function () {
      deprecationWarning(
        "ClippingPlanesEditor.readyPromise",
 ...
 * @private
 */
ClippingPlanesEditor.prototype._createPlanes = function (resolve, reject) {
  ...
  if (typeof target === "undefined") {
    const removeListener = this._scene.preRender.addEventListener(() => {
      ...
      removeListener();
      if (!this.isDestroyed()) {
        this._createPlanes(resolve, reject);
      }
    });
    return;
  }
  ...
  if (target instanceof Cesium3DTileset) {
    ...
    // This is here for backwards compatibility. It can be removed once Cesium3DTileset.readyPromise is removed and target can be assumed ready.
    target._readyPromise
      .then(() => {
    ...
    target.errorEvent.addEventListener(reject);
