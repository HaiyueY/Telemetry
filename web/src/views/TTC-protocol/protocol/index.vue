...
<template>
  <div class="app-container">
    ...
    <el-dialog
      ...
      :title="formData.id === undefined ? '新增协议' : '修改协议'"
      @closed="resetForm"
      @open="handleDialogOpen"
      width="40%"
    >
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="150px" label-position="right">
        <el-form-item prop="name" label="协议名称">
          <el-input v-model="formData.name" placeholder="请输入协议名称" />
        </el-form-item>
        <el-form-item prop="templateId" label="协议模板">
        ...
        <el-form-item v-if="formData.type !== ''">
          <el-select
            ...
            remote-show-suffix
            no-match-text="暂无数据"
            no-data-text="暂无符合条件的数据"
            placeholder="请选择协议模板"
            :remote-method="remoteSearchTemplate"
            :loading="selectTempLoading"
          >
            <el-option v-for="item in templates" :key="item.key" :label="item.label" :value="item.value" />
            <template #loading>
        ...
        <el-form-item prop="subsystemId" label="分系统(主类型)">
          <el-select
            v-model="formData.subsystemId"
            clearable
            filterable
            remote
            reserve-keyword
            ...
            :remote-method="remoteSearchSatellite"
            :loading="selectSatLoading"
          >
            <el-option v-for="item in satellites" :key="item.key" :label="item.label" :value="item.value" />
            <template #loading>
              <svg class="circular" viewBox="0 0 50 50">
                <circle class="path" cx="25" cy="25" r="20" fill="none" />
              </svg>
        ...
        <el-form-item prop="rec" label="版本标识(副类型)">
          <el-input v-model="formData.rec" placeholder="请输入版本标识(副类型)" />
        </el-form-item>
